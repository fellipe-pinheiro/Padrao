...
class Personalizado_modelo_dimensao_m extends CI_Model {
    ...
    public function get_by_id($id){
        $this->db->where('id', $id);
        $this->db->limit(1);
        $result = $this->db->get('personalizado_modelo_dimensao');
    ...
    public function inserir($dados) {
        if (empty($dados['id'])) {
            if ($this->db->insert('personalizado_modelo_dimensao', $dados)) {
    ...
    public function editar($dados) {
        if (!empty($dados['id'])) {
            ...
            if ($this->db->update('personalizado_modelo_dimensao', $dados)) {
                return true;
            }
        }
        return false;
    ...
    public function deletar($id) {
        if (!empty($id)) {
            $this->db->where('id', $id);
            if ($this->db->delete('personalizado_modelo_dimensao')) {
                return true;
            }
        }
        return false;
    ...
    private function changeToObject($result_db) {
        ...
        foreach ($result_db as $key => $value) {
            $object = new Personalizado_modelo_dimensao_m();
            $object->id = $value['id'];
    ...
    public function get_pesonalizado($id_modelo,$colunas)
    {
        ...
        return $this->db->get("personalizado_modelo_dimensao")->result_array();
    }
}