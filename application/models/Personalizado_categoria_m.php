...
class Personalizado_categoria_m extends CI_Model {

    var $id;
    var $nome;
    var $descricao;
    // Ajax 
    var $table = 'personalizado_categoria';
    ...
    public function get_by_id($id){
        ...
        $result =  $this->Personalizado_categoria_m->_changeToObject($result->result_array());
        return $result[0];
    }
    ...
    public function get_list($id = '') {
        ...
        return $this->Personalizado_categoria_m->_changeToObject($result->result_array());
    }

    public function inserir(Personalizado_categoria_m $objeto) {
        if (!empty($objeto)) {
            $dados = array(
                'id' => $objeto->id,
                'nome' => $objeto->nome,
            ...
            if ($this->db->insert('personalizado_categoria', $dados)) {
    ...
    public function editar(Personalizado_categoria_m $objeto) {
        if (!empty($objeto->id)) {
            $dados = array(
                'id' => $objeto->id,
                'nome' => $objeto->nome,
            ...
            if ($this->db->update('personalizado_categoria', $dados)) {
    ...
    public function deletar($id = '') {
        if (!empty($id)) {
            $this->db->where('id', $id);
            if ($this->db->delete('personalizado_categoria')) {
    ...
    function _changeToObject($result_db = '') {
        ...
        foreach ($result_db as $key => $value) {
            $object = new Personalizado_categoria_m();
            $object->id = $value['id'];
