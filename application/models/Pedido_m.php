...
class Pedido_m extends CI_Model {
    ...
    var $column_order = array('ped.id','ped.data','cli_nome','cli_sobrenome','orc.id','orc.data','orc.data_evento','cli_email','cli_telefone','cli_cpf','cli_cnpj','cli_razao_social','cli_pessoa_tipo','evento_nome','loja_unidade','orc.descricao','ped.condicoes');
    var $column_search = array('ped.id','orc.id','cli.nome','cli.sobrenome','cli.email','cli.cpf','cli.telefone','cli.cnpj','cli.razao_social','asr.email','date_format(orc.data,"%d/%m/%Y")','date_format(ped.data,"%d/%m/%Y")','date_format(orc.data_evento,"%d/%m/%Y")','evt.nome','loj.unidade');
    var $order = array('ped.id'=>'asc');
    ...
    private function get_datatables_query() {
    	...
    	if($this->input->post('data_pedido')){
    		$this->db->where('date_format(ped.data,"%Y-%m-%d")', date_to_db($this->input->post('data_pedido')));
    	}
    ...
    public function get_by_id($id){
    	...
    	if($result->num_rows() > 0){
    		$result =  $this->Pedido_m->changeToObject($result->result_array());
    		return $result[0];
    	}
    	return false;
    }
    ...
    public function get_by_id_cliente($id){
        ...
        if($result->num_rows() > 0){
            return  $this->Pedido_m->changeToObject($result->result_array());
        }
        return false;
    }
