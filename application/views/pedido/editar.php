									...
									<tbody>
										...
										foreach ($convites as $key => $convite) {
											...
											<tr class="tr-cancelado-<?=$convite->cancelado?>">
												...
												<td>R$ <?=number_format($convite->calcula_unitario(),5,',','.')?></td>
										...
										foreach ($personalizados as $key => $personalizado) {
											...
											<tr class="tr-cancelado-<?=$personalizado->cancelado?>">
												...
												<td>R$ <?=number_format($personalizado->calcula_unitario(),5,',','.')?></td>
										...
										foreach ($produtos as $key => $container) {
											...
											<tr class="tr-cancelado-<?=$container->cancelado?>">
												...
												<td>R$ <?=number_format($container->calcula_unitario(),5,',','.')?></td>
							...
							<div class="panel-body">
								<div id="forma-pagamento-content">
									...
									<div class="form-group">
										...
										<select onfocus="atualiza_quantidade_parcelas()" name="qtd_parcelas" id="qtd_parcelas" class="form-control">
...
<script type="text/javascript">
	...
	function calcular_custos_adm(valor) {
		var custos_adm = 0;
		var porcentagem_total = 100 + assessor_comissao;
		custos_adm = parseFloat((valor * assessor_comissao) / porcentagem_total).toFixed(5);
		//comentado pela mudança nos cálculos 19/01/2017
		//console.log(custos_adm);
		//custos_adm = parseFloat((valor/100) * assessor_comissao).toFixed(3);
		//return Math.round(custos_adm);
		return custos_adm;
	}
