    ...
    <div class="panel-body">
        ...
        <div class="row">
            <div class="col-sm-12 table-responsive">
                <table id="tabela_impressao_cor" class="table display compact table-bordered " cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Nome</th>
...
<div class="modal fade" id="modal_form">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?= form_open("#", 'class="form-horizontal" id="form_impressao_cor" role="form"') ?>
            <div class="modal-body form">
                ...
                <!--Referência-->
                <div class="form-group">
                    <?= form_label('Referência: ', 'referencia', array('class' => 'control-label col-sm-2')) ?>
                    <div class="col-sm-10">
                        <?= form_input('referencia', '', 'id="referencia" class="form-control" placeholder="Referência"') ?>
                        <span class="help-block"></span>
                    </div>
                </div>
                <!--Descrição-->
...
<script type="text/javascript">
    $(document).ready(function() {
        tabela = $("#tabela_impressao_cor").DataTable({
            scrollX: true,
            scrollY:"500px",
            scrollCollapse: true,
            dom: 'lBfrtip',
            ...
            serverSide: true,
            ajax: {
                url: "<?= base_url('impressao_cor/ajax_list') ?>",
                type: "POST"
            },
            columns: [
            {data: "id","visible": true},
            {data: "nome","visible": true},
            {data: "referencia","visible": true},
            {data: "descricao","visible": true}
            ]
        });
        // Resaltar a linha selecionada
        $("#tabela_impressao_cor tbody").on("click", "tr", function () {
        ...
        $("#adicionar").click(function(event) {
            ...
            $('.modal-title').text('Adicionar impressao_cor'); // Definir um titulo para o modal
        ...
        $("#editar").click(function () {
            ...
            $.ajax({
                url: "<?= base_url('impressao_cor/ajax_edit/') ?>" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data)
                {
                    $.map(data.impressao_cor, function (value, index) {
                        $('[name="' + index + '"]').val(value);

                    });

                    $('#modal_form').modal('show');
                    $('.modal-title').text('Editar impressao_cor');
                },
        ...
        $("#deletar").click(function () {
            ...
            if (confirm("O registro: " + nome + " será excluido. Clique em OK para continuar ou Cancele a operação.")) {
                $.ajax({
                    url: "<?= base_url('impressao_cor/ajax_delete/') ?>" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function (data)
                    {
        ...
        $("#form_impressao_cor").submit(function (e) {
            reset_errors();
            $('#btnSubmit').text('Salvando...');
            $('#btnSubmit').attr('disabled', true);
            var url;
            if (save_method == 'add') {
                url = "<?php echo site_url('impressao_cor/ajax_add') ?>";
            } else {
                url = "<?php echo site_url('impressao_cor/ajax_update') ?>";
            }
            $.ajax({
                url: url,
                type: "POST",
                data: $('#form_impressao_cor').serialize(),
                dataType: "JSON",
                success: function (data)
                {
                    if (data.status)
