        ...
        <nav class="navbar navbar-default navbar-static-top" role="navigation">
            <div class="container-fluid">
                ...
                <div class="navbar-header">
                    <button type="button" class="navbar-toggle" data-toggle="collapse" data-target=".navbar-laser-menu">
                        <span class="sr-only">Toggle navigation</span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                        <span class="icon-bar"></span>
                    </button>
                    <div class="navbar-brand">Laser</div>
                </div>
                
                <!-- Collect the nav links, forms, and other content for toggling -->
                <div class="collapse navbar-collapse navbar-laser-menu">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="col-sm-12 table-responsive">
                    <table id="tabela_laser" class="table display compact table-bordered " cellspacing="0" width="100%">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Nome</th>
...
<div class="modal fade" id="modal_form">
    <form action="#" method="POST" role="form" class="form-horizontal" id="form_laser">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group input-padding">
                                    ...
                                    <input type="text" name="nome" id="nome" class="form-control" required="required" placeholder="Nome do laser" pattern=".{1,50}" title="Máximo de 50 caracteres">
                                    <span class="help-block"></span>
                                </div>
                            </div>
                            <!--minutos-->
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#tabela_laser tbody").on("click", "tr", function () {
        ...
        $("#adicionar").click(function(event) {
            ...
            $('.modal-title').text('Adicionar laser'); // Definir um titulo para o modal
        ...
        $("#editar").click(function () {
            ...
            $.ajax({
                url: "<?= base_url('laser/ajax_edit/') ?>" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data)
                {
                    $.map(data.laser, function (value, index) {
                    ...
                    $('.modal-title').text('Editar laser ID: '+id);
                },
        ...
        $("#deletar").click(function () {
            ...
            $.confirm({
                ...
                confirm: function(){
                    $.ajax({
                        url: "<?= base_url('laser/ajax_delete/') ?>" + id,
                        type: "POST",
                        dataType: "JSON",
                        success: function (data)
                        {
        ...
        $("#form_laser").submit(function (e) {
            disable_button_salvar();
            reset_errors();
            var url;
            if (save_method == 'add') {
                url = "<?php echo site_url('laser/ajax_add') ?>";
            } else {
                url = "<?php echo site_url('laser/ajax_update') ?>";
            }
            $.ajax({
                url: url,
                type: "POST",
                data: $('#form_laser').serialize(),
                dataType: "JSON",
                success: function (data)
                {
                    if (data.status)
    ...
    function reset_form() {
        $('#form_laser')[0].reset(); // Zerar formulario
        reset_errors();
    }
</script>