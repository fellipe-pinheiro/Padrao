    ...
    <div class="panel-body">
        ...
        <div class="row">
            <div class="col-sm-12 table-responsive">
                <table id="tabela_papel_catalogo" class="table display compact table-bordered " cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            <th>ID</th>
                            <th>Nome</th>
...
<div class="modal fade" id="modal_form">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?= form_open("#", 'class="form-horizontal" id="form_papel_catalogo" role="form"') ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        tabela = $("#tabela_papel_catalogo").DataTable({
            scrollX: true,
            scrollY:"500px",
            scrollCollapse: true,
            dom: 'lBfrtip',
        ...
        $("#tabela_papel_catalogo tbody").on("click", "tr", function () {
        ...
        $("#adicionar").click(function(event) {
            ...
            $('.modal-title').text('Adicionar papel_catalogo'); // Definir um titulo para o modal
        ...
        $("#editar").click(function () {
            ...
            $.ajax({
                url: "<?= base_url('papel_catalogo/ajax_edit/') ?>" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data)
                {
                    $.map(data.papel_catalogo, function (value, index) {
                        $('[name="' + index + '"]').val(value);

                    });

                    $('#modal_form').modal('show');
                    $('.modal-title').text('Editar papel_catalogo');
                },
        ...
        $("#deletar").click(function () {
            ...
            if (confirm("O registro: " + nome + " será excluido. Clique em OK para continuar ou Cancele a operação.")) {
                $.ajax({
                    url: "<?= base_url('papel_catalogo/ajax_delete/') ?>" + id,
                    type: "POST",
                    dataType: "JSON",
                    success: function (data)
                    {
        ...
        $("#form_papel_catalogo").submit(function (e) {
            reset_errors();
            $('#btnSubmit').text('Salvando...');
            $('#btnSubmit').attr('disabled', true);
            var url;
            if (save_method == 'add') {
                url = "<?php echo site_url('papel_catalogo/ajax_add') ?>";
            } else {
                url = "<?php echo site_url('papel_catalogo/ajax_update') ?>";
            }
            $.ajax({
                url: url,
                type: "POST",
                data: $('#form_papel_catalogo').serialize(),
                dataType: "JSON",
                success: function (data)
                {
                    if (data.status)
...
function reload_table() {
    ...
    function reset_form() {
        $('#form_papel_catalogo')[0].reset(); // Zerar formulario
